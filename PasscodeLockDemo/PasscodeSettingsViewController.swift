...
class PasscodeSettingsViewController: UIViewController {
    ...
    @IBAction func passcodeSwitchValueChange(sender: UISwitch) {
        ...
        if passcodeSwitch.on {
            ...
            passcodeVC = PasscodeLockViewController(lockState: .SetPasscode, configuration: configuration)
            
        } else {
            
            passcodeVC = PasscodeLockViewController(lockState: .EnterPasscode, configuration: configuration)
    ...
    @IBAction func changePasscodeButtonTap(sender: UIButton) {
        ...
        let passcodeLock = PasscodeLockViewController(lockState: .ChangePasscode, configuration: config)
